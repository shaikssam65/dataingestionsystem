...
import { ApiEvent, Config } from './types';
...
export class Database {
   ...
   */
  async bulkInsert(events: ApiEvent[]): Promise<number> {
  ...
  async saveShard(workerId: number, shard: {
    startTs: number;
    endTs: number;
    cursor: string | null;
  ...
  async loadShards(): Promise<Array<{
    workerId: number;
    startTs: number;
    endTs: number;
    cursor: string | null;
  ...
  async getMeta(key: string): Promise<string | null> {
    const result = await this.pool.query(
      'SELECT value FROM ingestion_meta WHERE key = $1', [key]
    );
  ...
  async getAllEventIds(batchSize = 100_000): Promise<string[]> {
    logger.info('Fetching all event IDs from DB...');
